...
const JoinGCEP = () => {
  ...
  return (
    <div className="py-12 md:py-16 px-4 sm:px-6 lg:px-8 bg-white text-center">
      <h2 className="text-3xl sm:text-4xl md:text-[48px] font-bold mb-3 md:mb-4 font-['Poppins']">
      ...
      <div className="w-full relative flex flex-col md:flex-row justify-center items-center gap-8 md:gap-4 lg:gap-16 xl:gap-20">
        {steps.map((step, index) => (
          <div
            key={index}
            className="relative w-full max-w-xs sm:max-w-sm md:max-w-[300px] lg:max-w-sm"
          >
            {/* Enhanced Dashed Connection Line */}
            {index < steps.length - 1 && (
              <>
                {/* Mobile: Vertical dashed line card */}
                <div className="md:hidden absolute top-full left-1/2 transform -translate-x-1/2 h-18 w-px">
                  <svg
                  viewBox="0 0 160 40"
                  fill="none"
            ...
            <div className="bg-white rounded-xl shadow-md p-6 w-full text-center relative z-10 mb-12 md:mb-0">
              ...
              <h3 className="text-lg sm:text-xl md:text-[22px] font-medium mb-2">
                {step.title}
              </h3>
